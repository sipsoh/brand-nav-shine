...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Always-visible provenance line: every data point names its source. */
  .srcline { display: flex; align-items: center; gap: 6px; width: 100%; margin-top: 12px;
             padding: 8px 0 0; border: 0; border-top: 1px solid rgba(11,11,11,.06);
  ...
  @media print {
    ...
    .pillbtn { display: none !important; }
    .srcline { color: #8b8d9c; } /* provenance prints — that is the point */
...
const tableName = (trace) => tableNames[trace.tableId] || trace.tableId;
...
// Every data point carries a visible source line; clicking opens the full trace.
const srcLine = (title, trace) => {
  const button = el('button', 'srcline');
  button.title = 'View full source';
  button.appendChild(el('span', null, '⌕'));
  button.appendChild(el('span', 'txt',
    tableName(trace) + ' · ' + trace.calculation + ' · ' +
...
for (const widget of kpis) {
  ...
  if (kpi.sourceTrace) card.appendChild(srcLine(kpi.label, kpi.sourceTrace));
...
for (const chartSection of chartSections) {
  ...
  const addPanel = (widget, span) => {
    ...
    if (widget.chart.sourceTrace) {
      panel.appendChild(srcLine(widget.title, widget.chart.sourceTrace));
    }
